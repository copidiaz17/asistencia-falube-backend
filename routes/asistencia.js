...
// GET /api/obras/:obraId/asistencia/reporte?desde=YYYY-MM-DD&hasta=YYYY-MM-DD
router.get("/:obraId/asistencia/reporte", authMiddleware, hasRole([ROLES.ADMIN, ROLES.OPERATOR, ROLES.VIEWER]), async (req, res) => {
  try {
    ...
    if (!desde || !hasta)
      return res.status(400).json({ message: "Los parametros desde y hasta son requeridos." });
...
// GET /api/obras/:obraId/asistencia?fecha=YYYY-MM-DD
...
// POST /api/obras/:obraId/asistencia
