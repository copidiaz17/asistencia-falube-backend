// routes/estadisticas.js — Estadísticas de asistencia por empleado
...
/**
 * GET /api/estadisticas/empleado/:empleadoId?periodo=mes&year=2024&month=2
 * GET /api/estadisticas/empleado/:empleadoId?periodo=quincena&year=2024&month=2&quincena=1
 *
 * Retorna estadísticas de un empleado para un período específico
 */
  ...
  try {
    ...
    if (!periodo || !year || !month) {
      return res.status(400).json({
        ok: false,
        error: "Se requieren los parámetros: periodo, year, month"
      });
    }
    ...
    if (periodo === "quincena" && !quincena) {
      return res.status(400).json({
        ok: false,
        error: "Para período 'quincena' se requiere el parámetro quincena (1 o 2)"
      });
    }
    ...
    if (periodo === "mes") {
      ...
      fechaFin = new Date(yearNum, monthNum, 0); // Último día del mes
    } else if (periodo === "quincena") {
      // Quincena 1: día 1-15, Quincena 2: día 16 hasta fin de mes
      ...
      if (quincenaNum === 1) {
        ...
        return res.status(400).json({
          ok: false,
          error: "quincena debe ser 1 o 2"
        });
      }
    } else {
      return res.status(400).json({
        ok: false,
        error: "periodo debe ser 'mes' o 'quincena'"
      });
    }
    ...
    const asistencias = await Asistencia.findAll({
      where: {
        ...
        fecha: {
          [sequelize.Sequelize.Op.between]: [fechaInicioStr, fechaFinStr]
        }
      },
      order: [['fecha', 'ASC']]
    });
    ...
    asistencias.forEach(asist => {
      if (asist.presente) {
        ...
        // Calcular horas trabajadas si hay horarios
        if (asist.horarioIngreso && asist.horarioSalida) {
          ...
          let minutosTrabajados = minutosSalida - minutosIngreso;
          if (minutosTrabajados < 0) {
            minutosTrabajados += 24 * 60; // Cruzó medianoche
          }
    ...
    // Días sin registrar (no hay registro de asistencia ni inasistencia)
    ...
    const response = {
      ...
      detalle: asistencias.map(a => ({
        ...
        horasTrabajadas: (a.horarioIngreso && a.horarioSalida)
          ? calcularHoras(a.horarioIngreso, a.horarioSalida)
          : 0,
